// microbox.go contains logic to use microbox-router as a proxy
...
type Microbox struct{}

func (self Microbox) Init() error {
	// we want to see microbox-router logs
	...
	// set a default cert (*.microbox.app) self signed
...
func (self Microbox) SetRoute(route core.Route) error {
...
func (self Microbox) DeleteRoute(route core.Route) error {
	routes := router.Routes()
	for i := range routes {
		if routes[i].SubDomain == route.SubDomain && routes[i].Domain == route.Domain && routes[i].Path == route.Path {
...
func (self Microbox) SetRoutes(routes []core.Route) error {
	return router.UpdateRoutes(self.rToRoutes(routes))
}

func (self Microbox) GetRoutes() ([]core.Route, error) {
...
func (self Microbox) SetCert(cert core.CertBundle) error {
	certs := router.Keys()
	// for idempotency
	for i := range certs {
...
func (self Microbox) SetDefaultCert(cert core.CertBundle) error {
	return router.SetDefaultCert(cert.Cert, cert.Key)
}

func (self Microbox) DeleteCert(cert core.CertBundle) error {
	certs := router.Keys()
	for i := range certs {
		if certs[i].Cert == cert.Cert && certs[i].Key == cert.Key {
...
func (self Microbox) SetCerts(certs []core.CertBundle) error {
	return router.UpdateCerts(self.cToKeys(certs))
}

func (self Microbox) GetCerts() ([]core.CertBundle, error) {
	return self.keysToC(router.Keys()), nil
}

func (self Microbox) cToKeys(certs []core.CertBundle) []router.KeyPair {
...
func (self Microbox) keysToC(certs []router.KeyPair) []core.CertBundle {
...
func (self Microbox) rToRoutes(routes []core.Route) []router.Route {
...
func (self Microbox) routesToR(routes []router.Route) []core.Route {
	var rts []core.Route
	for i := range routes {
...
func (self Microbox) cToKey(cert core.CertBundle) router.KeyPair {
	return router.KeyPair{Key: cert.Key, Cert: cert.Cert}
}

func (self Microbox) rToRoute(route core.Route) router.Route {
